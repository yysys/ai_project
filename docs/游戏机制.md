...
function checkWinCondition(level: Level): boolean {
  ...
  // 检查是否所有狼都消失了
  const allWolvesDisappeared = level.wolves.every(wolf => wolf.isDisappeared);

  // 检查是否达到目标分数
  const reachedTargetScore = level.targetScore ? currentScore >= level.targetScore : true;

  return dogAlive && allWolvesDisappeared && reachedTargetScore;
}
```
...
function checkLoseCondition(level: Level): boolean {
  ...
  return dogDisappeared || timeExceeded;
}
```
...
### 15.1 方向指示器
```javascript
function drawDirectionIndicator(unit: Unit): void {
  // 在单位上方显示方向指示器
  const angle = DIRECTION_ANGLES[unit.direction];
  
  ctx.save();
  ctx.translate(unit.x, unit.y - unit.size / 2 - 20);
